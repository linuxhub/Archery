...
def archive_list(request):
    ...
    # 拥有审核权限、可以查看组内所有工单
        ...
        archive_config = archive_config.filter(group_id__in=group_ids)
    # 其他人只能看到自己提交的工单
    else:
        archive_config = archive_config.filter(user_name=user.username)
...
@permission_required('sql.archive_audit', raise_exception=True)
